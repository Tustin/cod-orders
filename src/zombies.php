...
file_put_contents($root . '/orders/' . date("mdY") . '_zombies.json', $orders);

$orders = json_decode($orders);

if (!$orders->data || $orders->status != "success" || !count($orders->data->Achievements)) throw new exception("bad order data");

$image = imagecreatefrompng($assets . "/templates/orders_zombies.png");
...
$order_image_name = $root . '/orders/' . date("mdY") . '_zombies.png';
imagepng($image, $order_image_name);

if (file_exists($order_image_name)) {
    //die("stopped");
    $tweet_text = "#WWIIZombies Orders for ". date("l, F jS, Y");
